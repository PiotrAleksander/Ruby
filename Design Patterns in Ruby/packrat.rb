...
class Backup
  ...
  def backup(dir, find_expression=All.new)
    puts "Wywołano metodę backup, katalog źródłowy: #{dir} wyrażenie: #{find_expression}"
    @data_sources << DataSource.new(dir, find_expression)
  end
  
  def to(backup_directory)
    puts "Wywołano metodę to, katalog kopii: #{backup_directory}"
    @backup_directory = backup_directory
  end
  
  def interval(minutes)
    puts "Wywołano metodę interval, okres: #{minutes} minut"
    @interval = minutes
  end
