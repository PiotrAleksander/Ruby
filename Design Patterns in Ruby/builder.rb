...
class ComputerBuilder
  ...
  def computer
    raise "Za mała kwota pieniężna" if @computer.motherboard.memory_size < 250
	raise "Zbyt wiele napędów" if @computer.drives.size > 4
	hard_disk = @computer.drives.find {|drive| drive.type == :hard_disk}
	raise "Brak dysku twardego." unless hard_disk
	@computer
  end
...
class LaptopBuilder < ComputerBuilder
  ...
  def display=(display)
    raise "Laptopy muszą korzystać z ekranów LCD." unless display == :lcd
  end
