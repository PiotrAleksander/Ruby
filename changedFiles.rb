...
Find.find(root) do |file|
  ...
  begin
    ...
    puts "Błąd odczytu pliku #{file} --- nie obliczono kodu MMD5."
  end
end
...
newfile_hash.each do |file, md5|
  report.puts "#{oldfile_hash[file] ? "Zmodyfikowano" : "Dodano"} plik: #{file}    #{md5}"
  oldfile_hash.delete(file)
end

oldfile_hash.each do |file, md5|
  report.puts "Usunięto/Przeniesiono plik: #{file} #{md5}"
end
